...
class PhotozXGBoost:
    ...
    def fit(self):
        ...
        model and config to ``save_dir/experiment_name`` when
    ...
    def load_model(self, model_path: str = None):
        ...
        Args:
            model_path: Path to the ``.json`` file. Defaults to
                ``save_dir/xgb_model.json``.
        """
    ...
    def __init__(self, model_dir: str):
        ...
        Args:
            model_dir: Path to the experiment directory containing
                ``config.yaml``, ``xgb_model.json``, and optionally
                ``scaler.pkl``.

        Raises:
            FileNotFoundError: If ``config.yaml`` or ``xgb_model.json`` is
                missing.
        """
...
#     dataset_mode={
#         'type': 'ratio',
#         'data_path': "/path/to/catalog.fits",
