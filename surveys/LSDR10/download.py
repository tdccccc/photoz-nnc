"""Bulk downloader for DESI Legacy Surveys DR10 galaxy photometry via NOIRLab Datalab.

The sky is tiled into an RA/Dec grid (see :func:`make_seg`). For each tile a
SQL query is submitted to the Datalab ``queryClient`` against ``ls_dr10.tractor``
joined with the ``photo_z`` tables. Only non-PSF (i.e. extended / galaxy)
sources with valid detections in g, r, z, W1, W2 are selected; Gaia astrometry
columns are included for downstream star/galaxy separation.

Photo-z source handling
-----------------------
The ``ls_dr10.photo_z`` table only covers the southern footprint
(dec < 32.375). For the northern strip (dec >= 32.375) the script
falls back to ``ls_dr9.photo_z``. Tiles that straddle the boundary
use a double LEFT JOIN with COALESCE, preferring DR10 when available.

Large-tile splitting
--------------------
Before downloading, each tile's row count is checked. If it exceeds
``MAX_ROWS_PER_REGION`` (5 million), the tile is recursively sub-split
along RA so that each piece fits in memory and Datalab's result-size
limit.

Failure tracking
----------------
Tiles that return zero rows are logged to ``empty_region_idx.txt`` and
skipped on subsequent runs. Tiles that error out are logged to
``failed_region_idx.txt`` (and retried on the next run). Both files use
file-level locking for safe concurrent writes from the process pool.

Note:
    For modest query volumes, TOPCAT's TAP client may be more convenient.
    This script is designed for the full-sky bulk download where TAP's
    interactive workflow becomes impractical.

Dependencies and setup
----------------------
Requires the NOIRLab Datalab client library (``astro-datalab``) and
``filelock``::

    pip install astro-datalab filelock

After installation, create a Datalab account at
https://datalab.noirlab.edu/ and replace the ``<YOUR_USERNAME>`` /
``<YOUR_PASSWORD>`` placeholders in the ``ac.login()`` call below with
your credentials (or use ``ac.login()`` interactively to cache a
token).
...
# DESI north/south boundary: dec < 32.375 is southern footprint (DR10
# photo_z), dec >= 32.375 is northern footprint (DR9 photo_z only).
DEC_BOUNDARY = 32.375
